...
Low fidelity models are those models that include no physics at run time. These types of models work for systems that have a set pattern of behavior. For example, recipe reactors use only input and output recipes. This means that they can simulate a reactor with a single computational opperation.  Some low fidelity models include interpolations techniques to build libraries on the fly. For example, interpolating between recipes for 3\% enriched fuel, and 4\% enriched fuel might be used to get a 3.5\% enriched fuel recipe. s
...
step specified is reached. For this work, the pieces of code that are used to provide the necessary functionality are OpenMC, Pyne, and Origen2.2. This flow is displayed in Figure \ref{fig:flow}.
...
reactor oriented calculations, and the ability to compute scattering kernels.
Currently, XSGen only requires the group flux values from OpenMC, and it extracts these each timestep. Input templates for OpenMC can be specified in the XSGen run control file. 

Standard templates may also be added to XSGen itself for use in future modeling. New templates can be added to the XSGen library of reactor types by creating a new input file for OpenMC that simulates the choice reactor type.
...
Generating a recipe for a recipe reactor is done using a full reactor lifetime. One technique is to use a high fidelity model to accurately model the burnup and transmutation of a reactor core from loading to discharge a fuel assembly. 

In addition to tracking a kilogram of each isotope in the reactor fuel, XSgen also tracks a kilogram of fuel through the reactor. When this fuel goes subcritical it represents a single batch reactor core going subcritical and needing to be refueled. However, it is possible to predict the burnup of a multi-batch core from the burnup of a single batch core. The following formula shows how this might be done. 
...
One thing to note about this technique is that it assumes that all batches are independent of each other. Additionally, this formula works best for reactors that have a decreasing criticality over a cycle. It would not work to provide a recipe for an accelerator driven system, or a breeder reactor. As these systems aim to maintain a criticality of one as the fuel burns by breeding in new fissionable material. 

\section{Results}
\subsection{Generation results}
Figures \ref{fig:u235xs} and \ref{fig:u238xs} cross section behavior results from running XSgen for a light water reactor using the inputs found in Table \ref{tab:xsgenstats1}. The fuel used was 3.5\% enriched uranium oxide (UOX). The number of energy groups used for this analysis was 1000. 
...
\begin{tikzpicture}
\begin{axis}[
    title={Cross Section Behavior of U235 in 3.5\% enriched LWR},
    xlabel={Days at 3E14 flux},
    ylabel={Barnes},
    xmin=0, xmax=6000,
    ymin=0, ymax=70,
...
\begin{tikzpicture}
\begin{axis}[
    title={Cross Section Behavior of U238 in 3.5\% enriched LWR},
    xlabel={Days at 3E14 flux},
    ylabel={Barnes},
    xmin=0, xmax=6000,
    ymin=0, ymax=1,
...
\begin{figure}[h]
  \center
  \includegraphics[scale=0.4]{fluxes.svg}
...
XSGen by itself can not directly be tested against operating reactor systems. It was originally designed to produce datasets a medium fidelity reactor model known as Bright-lite. Therefore, to benchmark XSGen it will be used in conjunction with Bright-lite to produce burnups and isotopes for several known reactor systems.  As Bright-lite has already been benchmarked several times using other datasets [BRIGHT-LITE Benchmark Citations]\citeme it is assumed that the operations within Bright-lite are accurate and any deviations within the results come from XSGen.

The most well studied reactors in the world are light water reactors and as such these will be used as benchmarking tools for XSGen. Primarily the cases represent a range of different enrichments in several light water reactors, comparing the results from XSGen/Bright-lite to recipes from VISION. Additionally, the values will be tested against two cases modelling reactor startup behavior.  [vision citation / transition citation] \citeme.
...
Table \ref{tab:xsgenresults} shows a very strong agreement between VISION and Bright-light results using datasets created from light water reactors designed to these particular enrichments. Bright-light has been benchmarked [Bright-lite citations] \citeme against VISION previously using other datasets. This shows that given XSGen generated datasets allows for Bright-lite to match VISION to a 5\% tolence for a majority of isotopes. Those that exhibit the greatest deviations are high order transuranics.
