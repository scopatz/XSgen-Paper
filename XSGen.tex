...
PyNE is capable of reading in cross section data from both ACE\cite{ace} and ENDF\cite{endf} datasets. It is able to synthesize cross section data - that may exist with
...
\[ \vec{\phi_g} = P \cdot \vec{\phi_h} \tag{1} \label{eq1} \] 
...
\[ \sigma_{r,g}^i = \frac{\sum_{E_j=E_l}^{E_j=E_u}0.5 \cdot[\sigma(E_j)+\sigma(E_{j+1})]\cdot[E_{j+1}-E_{j}]}{E_u-E_l} \tag{2} \label{eq2} \]
