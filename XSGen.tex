...
\title{XSgen: A Cross Section Generation Methodology for Fuel Cycle Simulation}
...
\begin{abstract}
This work investigates a method for generating medium fidelity reactor model cross sections.
The methodology used here couples a neutron transport code with a depletion
calculator. Together the two can be used to generate time dependent group cross sections
for medium fidelity reactor models in fuel cycle simulation. This work analyzes XSgen, a software
that performs the functions mentioned
above. Addtionally, it is shown that XSgen is capable of creating datasets
for a medium fidelity reactor model known as Bright-lite. The results of this work show
that XSgen produces datasets that match NEA benchmarks for both uranium
based LWR, and MOX reactors.
\end{abstract}

\begin{keyword}
Group Cross Section Generation, Nuclear Fuel Cycle, Medium Fidelity
\end{keyword}
...
conditions and remove any ambiguity that might stem from unknown reactor specification in a 
...
Accurately generating cross section datasets used in fuel cycle simulation allows for the minimization
of mass flow errors between nuclear facilities, whether they use low or medium fidelity models.
While the work in this discussion does not address, in depth, the impact of XSgen on nuclear fuel cycles 
it is important to mention that this work sets up the basis for using XSgen as a tool for verifying fuel cycle 
simulation. Specifically XSgen is a useful tool for building the datasets to simulate possible transition
scenarios for nuclear reactor fleets, for example transitioning from a once through cycle to a mixed 
UOX/MOX cycle.   
...
Bright-lite\cite{brightlite}, and CyBORG\cite{cyborg}.

High fidelity models are constructed using neutronics calculations, depletion
solvers\cite{bateman1910, bateman}, or other coupled multiphysics algorithms. These models tend
...
At a minimum, the one-group cross sections must be the same.
...
A one thousand group, logarithimically spaced between 1E-9MeV and 10MeV, flux is 
extracted from OpenMC. This flux is measured in the fuel region of supplied 
reactor geometry. This flux is fed into a cross
...
The purpose of XSgen is not to simulate a single reactor for core loading or analysis,
but to build the datasets necessary to simulate a type of reactor
within a discrete facility fuel cycle simulator. In this case each
facility within the simulation represents an individual reactor and will behave accordingly. Whether
the model is medium or low fidelity, it may perform calculations or computations each time step. 
Using a high fidelity model in this situation can become computationally expensive. For example
simulations of the U.S. nuclear reactor fleet would require modeling roughly 100 reactors.

By using XSgen to produce the datasets for medium fidelity reactor models, the heavy computation
of a high fidelity model only needs to be performed once. The medium or low fidelity models
can use the resulting datasets to simulate that reactor repeatedly during the simulation. 

The automation inherit in XSgen allows users to built datasets for a single reactor type using
a variety of parameter ranges. For example instead of using a reactor with set parameters
a user of XSgen could build datasets for a medium fidelity reactor of that same design with
burnups of 30MWd/kgIHM, 40MWd/kgIHM, and 50MWd/kgIHM. These reactors would then be useable for
a fuel cycle analysis determining burnup's affect on high level waste volume. 
...
at discharge using reactor batch physics which is discussed in section \S\ref{sec:recipe}.

The following sections will demonstrate the workflow and capabilities of XSgen. The workflow described in section 
\S\ref{sec-workflow} shows both the computational work as well as the mathematical basis. 
Following that is a description of the direct cross section results from XSgen in section 
\S\ref{sec:crosssection}. Finally, XSgen is used for several verification cases with current reactor 
...
step is reached. This type of linkage has been made in other softwares, for example LOOP\cite{loop}. However, 
...
reactor oriented calculations\cite{serpentvmonte}, and its capability to compute scattering
...
and templating it according to a set of field variables that XSgen provides.
XSgen comes stock with standard templates representing pressurized water reactor (PWR)
...
displays the relations shown in Equation \ref{pem-relations}.
\begin{equation}
\label{pem-relations}
P_{g,h} = \left\{
\begin{array}{ll}
...
\sigma_{r,g}^i = \frac{1}{2} \frac{\sum_{E_p\ge E_{g+1}}^{E_p\le E_g}
                                   \left(\sigma_r^i(E_p)+\sigma_r^i(E_{p+1})\right)
...
\begin{equation}
\label{ssw}
\omega_{g}^i=\left(\frac{\sum_{j\neq i}N_j \, \sigma_{t,g}^j}{N_i}+\sigma_{t,g}^i\right)^{-1}
\end{equation}
...
the challenge in this circumstance is obtaining a pointwise flux. OpenMC does not output
...
Equation \ref{pointwise-flux} \cite{spectrum}. This flux is only used to calculate the multi-group
total cross sections used to calculate the self shielding weights. 
\begin{equation}
...
The flux used in equations \ref{ssww} and \ref{phiw} is the multi-group flux
obtained from the OpenMC calculation. 

The effects of self shielding on U-235 inside of a fuel pellet may be seen in
...
There exists a method of calculating these reaction cross sections within OpenMC that accounts
for all of this work, however the cross sections calculated are limited to the nuclides included
in the OpenMC cross section set. This methodology was included within XSgen to ensure that 
users may provide their own cross section dataset (for whatever reason) and get information
for each nuclide that they require. 

\subsection{Depletion via ORIGEN v2.2}
\label{sec:origen}

After a TAPE9 file (the cross section format of ORIGEN v2.2) has been constructed by XSgen, it is
...
providing one-group cross sections for the two reactor types tested. This methodology
is extendable to other types of nuclear reactors that can be modeled in OpenMC. 
...
needed for future fuel cycle analysis campaigns.

\section{Acknowledgements}
The preparers of this work would like to thank Dr. Erich Schneider, and Dr. Cem Bagdatlioglu for their
support. Their efforts provided the researchers with the knowledge on how to link the XSgen methodology
to the Bright-lite infrustructure. Additional thanks to the Nuclear Engineering University Program for
funding this work under project number 12-4065.
