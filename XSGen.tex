...
Simulation is an essential part of nuclear engineering. In regards to reactor model simulation, there are three main divisions of simulation type; low, medium, and high fidelity. Low fidelity models are those models that include no physics at run time. An example of low fidelity modelling are recipe reactors techniques. Medium fidelity models are those that include physics. Examples of these medium fidelity models are Class\citeme, and Bright-lite\citeme. High fidelity models are performed using neutronics calculations and bateman equation solvers. An example of a high fidelity reactor model is the MCNP\citeme burn card.   
